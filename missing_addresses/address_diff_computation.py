...
class AddressDiff:
    """
    Diff between reference and actual set of addresses.
    """
...
class AddressDiffMetaData:
    """Metadata for the diff between reference and actual set of addresses, e.g. dates of the input data."""
    area_name: str
...
class AddressDiffComputation:
    def compute(self, reference: Dict[Address, Coordinate], actual: List[Tuple[Address, OsmPrimitive]],
    ...
    def pretty_diff(self, reference, actual, ignored) -> AddressDiff:
        ...
        diff = self.compute(reference, actual, ignored)
